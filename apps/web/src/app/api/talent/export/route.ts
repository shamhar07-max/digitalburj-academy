...
export async function GET() {
  ...
  const evidence = all<{ id: number; skill: string; mission: string; at: string }>(
    `SELECT e.id, s.name AS skill, m.title AS mission, e.created_at AS at FROM evidence e
  ...
  return NextResponse.json({
    ...
    skills: skills.filter((s) => s.level > 0),
    evidence,
